...
app.post("/update", async (req, res) => {
  // Menggunakan async function untuk menangani promise
  try {
    const data = await Data.find({}); // Menggunakan await untuk menunggu promise selesai
    for (const item of data) {
      // Menggunakan for...of untuk mengulangi data secara berurutan
      const address = await Address.findOne({ co_name: item.co_name }); // Menggunakan await untuk menunggu promise selesai
      if (address) {
        // Membagi nilai co_lat dan co_lng dengan 10^7 untuk mendapatkan nilai desimal yang benar
        const co_lat = address.co_lat / 10000000;
        const co_lng = address.co_lng / 10000000;
        await Data.updateOne(
          { _id: item._id },
          { co_lat: co_lat, co_lng: co_lng }
        ); // Menggunakan await untuk menunggu promise selesai
        console.log("Updated location for " + item.sell_id);
        const googleMapsApiKey = "AIzaSyD4sgjH4RAaAokyujwQO_jSeZDowQ1U9Oo";
        // Menggunakan parameter result_type untuk membatasi jenis alamat yang diinginkan
        // Menggunakan parameter language untuk mendapatkan alamat dalam bahasa Indonesia
        const googleMapsApiUrl = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${co_lat},${co_lng}&result_type=street_address&language=id&key=${googleMapsApiKey}`;
        const response = await axios.get(googleMapsApiUrl); // Menggunakan await untuk menunggu promise selesai
      ...
      // Menggunakan parameter result_type untuk membatasi jenis alamat yang diinginkan
      // Menggunakan parameter language untuk mendapatkan alamat dalam bahasa Indonesia
      const googleMapsApiUrl = `https://maps.googleapis.com/maps/api/geocode/json?address=${destination}&result_type=street_address&language=id&key=${googleMapsApiKey}`;
      const response = await axios.get(googleMapsApiUrl); // Menggunakan await untuk menunggu promise selesai
      const results = response.data.results;
      if (results.length > 0) {
        const location = results[0];
        ...
        console.log("Updated address destination for " + item.sell_id);
      }
    }
    ...
    res
      .status(500)
      .send("Terjadi kesalahan saat memperbarui lokasi dari MongoDB");
  }
});
...
//               Data.updateOne(
//                 { _id: item._id },
//                 // Gunakan operator $set untuk mengupdate properti dokumen
//                 { $set: { co_lat: address.co_lat, co_lng: address.co_lng } }
//               )
//                 .then(() => {
//                   console.log("Updated location for " + item.sell_id);
...
//                           { _id: item._id },
//                           // Gunakan operator $set untuk mengupdate properti dokumen
//                           { $set: { co_add: location.formatted_address } }
//                         )
//                           .then(() =>
//                             console.log("Updated address for " + item.sell_id)
//                           )
...
//                           { _id: item._id },
//                           // Gunakan operator $set untuk mengupdate properti dokumen
//                           { $set: { coe_add: location.formatted_address } }
//                         )
//                           .then(() =>
//                             console.log(
